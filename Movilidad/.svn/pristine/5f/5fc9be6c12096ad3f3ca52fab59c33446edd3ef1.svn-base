...
public class DatabaseInitialLoader {
    ...
    public void init() throws Exception {
        ...
        try {
            ...
            //TODO: Se eliminara para generar los datos de otra manera
            generacionDatosTransportePublico();
    ...
    private void generacionDatosTransportePublico() {
        ...
        // TODO: Se eliminara
        cargarPersonasAbogados();
    }

    private void cargarPersonasAbogados() {

        loadCSVTableReferenceJpaRepository("personasAbogados.csv", Persona.class, PersonaRepo.class,
                (x, s) -> x.setId(UUID.randomUUID()),
                (x, s) -> x.setNumeroDocumentoIdentidad(s),
                (x, s) -> x.setPrimerApellido(s),
                (x, s) -> x.setPrimerNombre(s),
                (x, s) -> x.setSegundoApellido(s),
                (x, s) -> x.setSegundoNombre(s),
                (x, s) -> x.setTipoDocumentoIdentidad(tipoDocumentoRepo.findOne(Integer.parseInt(s))),
                (x, s) -> x.setTipoPersona(tipoPersonaRepo.findOne(Integer.parseInt(s)))
        );

        List<Persona> personas = personaRepo.findAll();
        for(Persona persona : personas){
            if(!persona.getPrimerApellido().equals("SIPA")){
                Abogado abogado = new Abogado();
                abogado.setId(UUID.randomUUID());
                abogado.setPersona(persona);
                abogado.setInicioVinculacion(new Date());
                abogado.setFinVinculacion(new Date());
