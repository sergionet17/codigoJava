angular.module('forest5').controller('aloneController', function($scope, $window, $http, $resource, tareas, tareasCompletar, procesos, instanciaprocesos, tareasCompletar, formas, usuarios1, tareasAsignar, CONFIG) {
    ...
    self.cargarTarea = function(tarea) {
        ...
        $scope.tarea_next = true;
        //console.log(1,JSON.stringify(tarea)+"---"+self.tareaActual.processInstanceId)
        //carga las variabes de instancia
//console.log("---"+self.tareaActual.processInstanceId )
        //http://54.197.37.166:8080/engine-rest/process-instance/74142842-25d1-11e7-98bf-0a35727e87ca/variables/
        if (!tarea) {
            $scope.tarea_next = true;
            return;
        }
        if (!tarea || self.tareaActual.processInstanceId ==  undefined) {
            $scope.tarea_name = tarea.name;
            $scope.tarea_next = false;
            return;
        }

        var urlConsultaVariables = CONFIG.url + '/' + CONFIG.contexto + '/process-instance/' + self.tareaActual.processInstanceId + '/variables/';
        console.log(2,urlConsultaVariables)
        console.log(CONFIG.urlFormas + '/' + CONFIG.contextoFormas + '/bycode/' + tarea.formKey)
        ...
        jQuery.getJSON(CONFIG.urlFormas + '/' + CONFIG.contextoFormas + '/bycode/prueba_ancho', function(result) {
            var jso = JSON.parse(result.xml);
                jso.parent = $("#sipa_well");
                xui.newCom("xui.RenderForm", function() {
                    {
                        ...
                        //console.log("formaa", JSON.parse(result.xml.height));
                        var array_js = JSON.parse(result.xml);
                        //console.log("<--->"+array_js.height) 
                        ...
                        var hx =$("#formxui").height();
                        console.log("Altura--"+array_js.height);
                        x.height(array_js.height+"px");
                        self.cambiaTab('test3');
                        
                        xui("formxui").append(this.rootPane);
                        
                       // $("#formxui").css("min-height",xh+'px');
                        return this;
                    }
                }, undefined, {
                    //form: result
    ...
    this.avanzaTarea = function() {
        ...
        for (var key in $scope.formaFinal) {
            ...
            if ($scope.formaFinal[key].getUIValue) {
                    ...
                    tipo = $scope.formaFinal[key].n0.properties.dataType;
                if(tipo !== undefined){
                    if (tipo == 'date')
